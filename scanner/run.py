...
def TimerCheck(IOLoop,proxytype='http'):
    def func(ip,port,proxytype,anonymous,connect_time):
        if session.query(Proxy).filter(Proxy.ip==ip,Proxy.port==port,Proxy.type==proxytype).first()==None:
            try:
                pos=IPFIND(ip).split('\t')
...
def progress_show(IOLoop):
    bar_length=20
    percent=IOLoop.scanned/IOLoop.sum_ips
...
proxyloop.addtimer(progress_show,3,once=False)
...
proxyloop.scanips(iplists,proxytype=2)
